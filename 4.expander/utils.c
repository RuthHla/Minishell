...
// 			j = i + 1;
// 			k = 0;
// 			len = strlen_variable(str, j);
// 			variable[index] = malloc(sizeof(char) * (len + 1));
// 			if (!variable[index])
// 				return (NULL);
...
int	how_many_variable(char *str)
{
	...
	while (i < len)
	{
		if (str[i] == '$')  // Correction: str[i] au lieu de sstr[i]
		{
			j = i + 1;
			if (j < len && valid_variable_char(str[j]))
			{
				n_var++;
				// Sauter le reste de la variable
				while (j < len && valid_variable_char(str[j]))
					j++;
				i = j - 1;
			}
		}
...
char	**find_variable_in_str(char *str)
{
	...
	while (i < str_len)
	{
		if (str[i] == '$' && i + 1 < str_len && valid_variable_char(str[i + 1]))
		{
			j = i + 1;
			k = 0;
			...
			while (valid_variable_char(str[j]))
				variable[index][k++] = str[j++];
			variable[index][k] = '\0';  // Important: terminer la string
			index++;
			i = j - 1; // Reprendre après la variable
		}
		i++;
	}
	variable[index] = NULL;
